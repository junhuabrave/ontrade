...
void push_book(MdRing& md, std::uint64_t inst, std::int64_t bid,
               std::int64_t ask, std::uint64_t seq) {
...
int main() {
    ...
    GatewayType gw(oms_to_venue, venue_to_oms, clk);
    ...
    // Seed quote.
    push_book(md, kInst, bid, ask, seq++);
    run_until_quiescent();
